...
export async function getNews(category, offset, limit) {
  const result = await fetch(`${urlCategory}category=${category}&offset=${offset}&limit=${limit}`).then(response => response.json());
...
export async function getArticleSearch(search, offset, limit) {
  const result = await fetch(`${urlSearch}search=${search}&offset=${offset}&limit=${limit}`).then(response => response.json());

  // Warning: It says NewsArticle because I didn't change backend to pass back the array as SearchArticle
  if (result.NewsArticle === undefined || (result.NewsArticle.length == 0 && offset == 0)) return { SearchArticle: result.NewsArticle, emptySearchReturned: true };
