...
import {
  NetInfo, Dimensions, Image, Platform, BackHandler, TouchableHighlight, TextInput, FlatList, StyleSheet, View, Text, SafeAreaView
} from 'react-native';
...
// Component used to render each entry in the list
...
export default class ObservationsListPage extends React.Component {
    ...
    fetchObservation = () => {
      this.state.user.getIdToken()
        .then((idToken) => {
		    	const completeUrl = `${urlObservation
    ...
    observationSearch = async (searchText) => {
	    const idToken = await this.state.user.getIdToken();
	    const urlComplete = `${urlPostSearch}search=${searchText}` + `&offset=${this.state.searchOffset}`
	    	+ `&limit=${LIMIT}` + `&idToken=${idToken}` + `&uid=${this.state.user.uid}`;
	    const result = await fetch(urlComplete);
...
const styles = StyleSheet.create({
  ...
  image: {
    ...
    width: Math.round(Dimensions.get('window').width * (0.94)),
    height: 200,
    resizeMode: 'cover',
    // borderWidth: 2,
    alignSelf: 'center'
