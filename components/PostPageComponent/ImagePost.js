...
import {
  View, StyleSheet, Image, Text, Platform, KeyboardAvoidingView
} from 'react-native';
  ...
  constructor(props) { // This function is needed otherwise props cannot be accessed
    ...
    handleUploadPhoto = () => {
      this.state.user.getIdToken()
        .then((idToken) => {
          ...
          const url = setUrlParam(URL, tempParam);
    ...
    handleGetUserLocation = () => {
      navigator.geolocation.getCurrentPosition((position) => {
        ...
        const currState = setUserLocation(this.state, null, null, 'init', true, 'Failed to Include User Location');
        this.setState({ currState });
      });
    };
    ...
    render() {
      ...
      return (
        <View style={{
          flex: 1, alignItems: 'center', justifyContent: 'center', backgroundColor: '#f9f5ed'
        }}
        >
          {photo && (
          <React.Fragment>
...
const setUrlParam = (url, param) => {
  const name = (param.name == undefined) ? 'default-name' : param.name;
  let resultUrl = `${url}?name=${name}`;
