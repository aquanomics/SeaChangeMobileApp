...
import {
  StyleSheet, Image, Text, View, ImageBackground, TouchableOpacity, ScrollView, ActivityIndicator
} from 'react-native';
...
export default class ProfilePage extends Component {
    ...
    constructor(props) {
      ...
      let externalDisplayDialog = this.props.navigation.getParam('externalDisplayDialog', undefined);
      if (externalDisplayDialog) externalDisplayDialog = true;
      else externalDisplayDialog = false;
    ...
    onPressLogIn = () => {
      ...
      if (missingArr.length > 0) { // if there are any missing fields, notify them to the user
        const missingStr = missingArr.join(' and ');
        const errorStr = `Error: ${missingStr} ${missingArr.length == 1 ? 'field is' : 'fields are'} empty`;
    ...
    turnOnFirebaseAuthCallback = () => {
      this.unsubscriber = firebase.auth().onAuthStateChanged((user) => {
        if (user) {
          user.getIdToken()
            .then((idToken) => {
              const urlString = `${urlUserFetch}uid=${user.uid}` + `&idToken=${idToken}`;
...
function DisplayAccountInfo(props) {
  ...
  } if (props.screenState == 'logged_in') { // else, user must be signed in
    return (
