...
import {
  View, Text, StyleSheet, ImageBackground, BackHandler
} from 'react-native';
...
export default class SignupPage extends Component {
    ...
    onPressSignUp = () => {
      ...
      if (missingArr.length > 0) {	// if there are any missing fields, notify them to the user
        const missingStr = missingArr.join(', ');
        ...
        fetch(`${urlUsernameCheck}?username=${this.state.username}`)
          .then((response) => {		// 1st then
            // invalid checking
            if (response.status != 200) throw ({ message: `Internal server error! Error code ${response.status}` });
